...
export function SettingsModal(onClose) {
    ...
    apiPanel.innerHTML = `
        <div style="display:flex;flex-direction:column;gap:0.75rem;">
            <div>
                <label style="display:block;font-size:0.75rem;color:rgba(255,255,255,0.5);margin-bottom:0.4rem;font-weight:600;">${t('settings.veniceKeyLabel')}</label>
                <input id="settings-venice-key" type="password"
                    ...
                    placeholder="${t('settings.veniceKeyPlaceholder')}"
                    value="${localStorage.getItem('venice_api_key') || ''}">
            </div>
            <div>
                <label style="display:block;font-size:0.75rem;color:rgba(255,255,255,0.5);margin-bottom:0.4rem;font-weight:600;">${t('settings.openrouterKeyLabel')}</label>
