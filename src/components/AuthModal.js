...
export function AuthModal(onSuccess) {
    ...
    modal.innerHTML = `
        ...
        <div class="space-y-4">
            <div class="space-y-2">
                <label class="text-[10px] font-bold text-muted uppercase tracking-widest ml-1">${t('auth.veniceKeyLabel')}</label>
                <input 
                    type="password" 
                    id="venice-key-input"
                    placeholder="${t('auth.veniceKeyPlaceholder')}"
            ...
            <div class="space-y-2">
                <label class="text-[10px] font-bold text-muted uppercase tracking-widest ml-1">${t('auth.openrouterKeyLabel')}</label>
                <input 
                    type="password" 
                    id="openrouter-key-input"
                    placeholder="${t('auth.openrouterKeyPlaceholder')}"
                    class="w-full bg-black/40 border border-white/5 rounded-2xl px-5 py-4 text-white placeholder:text-muted focus:outline-none focus:border-primary/50 transition-colors shadow-inner"
                >
            </div>
            ...
            <div class="flex flex-col gap-3 mt-4">
                ...
                <div class="flex justify-between px-2">
                    <a href="https://venice.ai" target="_blank" rel="noopener noreferrer" class="text-center text-[11px] font-bold text-muted hover:text-white transition-colors py-2 uppercase tracking-tighter">
                        ${t('auth.getVeniceKey')}
                    </a>
                    <a href="https://openrouter.ai" target="_blank" rel="noopener noreferrer" class="text-center text-[11px] font-bold text-muted hover:text-white transition-colors py-2 uppercase tracking-tighter">
                        ${t('auth.getOpenrouterKey')}
                    </a>
                </div>
            </div>
        </div>
    ...
    // Populate existing keys if any
    const existingVenice = localStorage.getItem('venice_api_key');
    const existingOpenRouter = localStorage.getItem('openrouter_api_key');
    if (existingVenice) veniceInput.value = existingVenice;
    if (existingOpenRouter) openrouterInput.value = existingOpenRouter;
